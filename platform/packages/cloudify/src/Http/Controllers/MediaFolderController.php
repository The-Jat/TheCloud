...
class MediaFolderController extends BaseController
{
    ...
    public function show(string $folder, MediaFileInterface $fileRepository): BaseHttpResponse
    {
        $this->checkAbility(ExternalAbility::VIEW_MEDIA_FOLDER);

        $files = $fileRepository->getFilesByFolderId($folder)->where('is_folder', false);
        $folder = MediaFolder::query()->findOrFail($folder);

        return $this
            ->httpResponse()
            ->setData(FolderResource::make($folder)->withFiles($files));
    }
